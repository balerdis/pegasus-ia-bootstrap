...
`sdd-tasks` owns an atomic writer thread: edits, full reread, language/marker/source/work-unit/seven-line forecast/exact component and aggregate estimate/dependency-graph/test validation, revision freeze, then `record_task_progress` and exactly one `record_handoff` for the final revision. Invalid estimates require correction plus a new full reread/revalidation before freeze, or a blocked result. It returns one render-safe canonical JSON v2 result block with all string slashes encoded as `\u002f` and a digest over the exact serialized bytes. The orchestrator rejects legacy, mixed, malformed, mutated, or non-canonical blocks, validates the digest and decoded canonical path, and copies the complete block byte-for-byte without reconstructing fields. High, chained, decision-required, or over-budget forecasts block apply until the user explicitly selects `stacked-to-main`, `feature-branch-chain`, or maintainer-approved `size:exception`; tasks-only intent and historical preferences cannot bypass that current decision.
