...
`sdd-tasks` owns an atomic writer thread: edits, full reread, language/marker/source/work-unit/seven-line forecast/component-estimate/dependency-graph/test validation, revision freeze, then `record_task_progress` and exactly one `record_handoff` for the final revision. It returns one delimited immutable result block carrying all validations and forecast values, canonical path, work-unit scope, matching revisions, exact `Post-persistence edits: none`, recovery transitions, truthful persistence states, observable handoff invocation identity, risks, decision, next action, and a payload digest. The orchestrator validates block identity/schema/path and copies the complete block byte-for-byte without reconstructing fields. High, chained, decision-required, or over-budget forecasts block apply until the user explicitly selects `stacked-to-main`, `feature-branch-chain`, or maintainer-approved `size:exception`; tasks-only intent and historical preferences cannot bypass that current decision.
