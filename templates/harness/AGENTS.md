...
`sdd-tasks` owns an atomic writer thread: edits, full reread, language/marker/source/work-unit/seven-line forecast/estimate/test validation, revision freeze, then `record_task_progress` and `record_handoff`. Its flat envelope carries all validations and forecast values, work-unit scope, matching revisions, exact `Post-persistence edits: none`, recovery transitions, truthful persistence states, risks, decision, and next action. The orchestrator reproduces and consumes it field-for-field. High, chained, decision-required, or over-budget forecasts block apply until the user explicitly selects `stacked-to-main`, `feature-branch-chain`, or maintainer-approved `size:exception`; tasks-only intent and historical preferences cannot bypass that current decision.
