...
Treat completed-path closure as one atomic sequence: finish every artifact edit, repair, formatting rewrite, and content mutation; reread the complete artifact; validate markers, language/terminology, per-entry traceability, complete proposal-risk design/test or measurement coverage, deferred-choice disposition, and source evidence; then freeze the final artifact identity. Use a content hash when observable, otherwise an explicit stable revision token bound to that validated read. No artifact mutation is permitted after the first persistence operation begins.
...
**Completed path:** only after marker, language, and gap validation, all edits, full reread, every remaining validation, and final artifact identity freeze, satisfy `ensure_project` and `ensure_change` preconditions before writes, then call/attempt exactly `record_artifact`, `record_observation`, `record_task_progress` for phase `design`, and `record_handoff`, in that order. Every affected persistence record MUST carry or reference the frozen final artifact revision. Use `completed` only when the design is ready for review with no blocking gaps; a non-blocking deferred choice may still be `completed` with notes. The progress record identifies the artifact path, frozen revision, every deferred choice (or `None` / `Ninguna`), its needed-by gate, blockers/gaps, and next action `review/approval`. Only after those operations may the specialist construct and return the envelope. On successful closure, report the identical final and persistence revisions and exactly `Post-persistence edits: none`.

The completed path always performs `record_task_progress` for phase `design` before `record_handoff`.
...
If any artifact edit occurs or becomes necessary after persistence begins, all earlier completion and persistence evidence is stale. Immediately block success, finish the edit, repeat the full reread and every validation, freeze a new final identity, and refresh every affected persistence record in the exact `record_artifact` → `record_observation` → `record_task_progress` → `record_handoff` order after ensure preconditions. Never return `completed`, reuse pre-edit records, or claim `Post-persistence edits: none` until the refreshed persistence revision equals the final artifact revision and no later mutation occurred.
