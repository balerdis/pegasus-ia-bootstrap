...
For tasks, immutable-block transport supersedes any field-by-field reproduction wording below. Require exactly one `=== PEGASUS SPECIALIST RESULT BEGIN v2 ===`/`=== PEGASUS SPECIALIST RESULT END v2 ===` pair, one `Specialist result block revision`, and one canonical JSON line in the exact specialist-defined key order. Reject any v1 delimiter, mixed/duplicate block, extra line, raw slash in a JSON string, duplicate/unknown/missing/reordered key, malformed escape, lone surrogate, or non-string field value. Recompute lowercase SHA-256 over the exact UTF-8 JSON-line bytes plus one final LF before decoding and require identity with the returned revision. Then strictly decode, require schema `pegasus-specialist-result/v2`, reserialize canonically with every `/` encoded as lowercase `\u002f`, and require byte identity with the received JSON line. Validate decoded `artifact_path` against the path constructed from the active change, matching artifact revisions, `post_persistence_edits` exactly `none`, truthful persistence states, and authorized strategy evidence. Missing/duplicate delimiters, mutation, legacy/mixed content, a second reconstructed envelope or artifact-path field outside it, decoded path mismatch, non-canonical serialization, or digest mismatch blocks success and the strategy/apply guard.
...
Copy the entire validated four-line v2 tasks block into the final user-facing response byte-for-byte, including both delimiters, digest, canonical JSON line, and line endings. Do not reconstruct an envelope outside it. After the block, strategy narrative may state only the guard consequence derived from decoded values without quoting keys or reconstructing fields. Then reproduce exactly this single canonical Spanish strategy question, with no prefix, suffix, interpolation, or paraphrase: `La previsión requiere definir la estrategia antes de apply. ¿Elegís \`stacked-to-main\`, \`feature-branch-chain\` o una excepción \`size:exception\` aprobada por el maintainer? No se iniciará apply hasta que respondas.` A request that asks only for tasks still requires this post-tasks review-guard question; it is not apply execution. A generic pause, a question without all three exact options, a silent/default choice, a duplicated/reconstructed envelope, or a question issued after an invalid block is invalid.
...
If block validation fails, return a blocked report outside specialist-result delimiters, name the exact integrity failure, and do not ask strategy or launch apply.
