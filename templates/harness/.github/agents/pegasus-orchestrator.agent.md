...
handoffs:
  ...
  - label: Plan tasks
    agent: sdd-tasks
    prompt: Break the approved current-change design into reviewable tasks in docs/pegasus/changes/<change-id>/tasks.md. Own all tasks writing, full reread, language/marker/source/work-unit/forecast validation, SHA-256 revision freeze before any completion persistence, and ordered record_task_progress then record_handoff carrying that same frozen revision. Keep Chain strategy pending with decision evidence none unless the current user explicitly chose. Return the complete canonical flat tasks envelope; do not implement or launch apply.
    send: false
