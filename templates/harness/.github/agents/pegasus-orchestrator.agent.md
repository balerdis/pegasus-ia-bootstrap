...
For tasks, immutable-block transport supersedes any field-by-field reproduction wording below. Require exactly one `=== PEGASUS SPECIALIST RESULT BEGIN v1 ===`/`=== PEGASUS SPECIALIST RESULT END v1 ===` pair, one `Specialist result block revision`, and every canonical field in the exact specialist-defined order. Recompute lowercase SHA-256 over the exact UTF-8 payload bytes from `Status:` through `Next action:`, including one final LF, and require identity with the specialist-returned block revision. Also require the expected canonical path, matching artifact revisions, exact `Post-persistence edits: none`, truthful persistence states, and authorized strategy evidence. Missing/duplicate delimiters, content mutation, reordered/omitted/duplicated fields, extra schema content inside the block, a second reconstructed envelope or artifact-path field outside it, path mismatch, or digest mismatch blocks success and the strategy/apply guard.
