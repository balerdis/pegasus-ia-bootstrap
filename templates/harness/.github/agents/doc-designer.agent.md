...
Before phase work, require the dispatch payload's exact canonical project key and launch identity, exactly one product request, and exactly one PRD artifact: either `docs/pegasus/prd.md` or `docs/pegasus/changes/<change-id>/prd.md`. Require explicit authorization to discover or refine it. Missing, mismatched, ambiguous, or contradictory identity/authorization blocks before any edit or persistence; never derive project identity from product prose, title, or path.
