...
Before phase work, require explicit payload fields `project_key` and `launch_identity`, exactly matching the manifest and authorized PRD identity, plus one product request, one PRD artifact, and discovery/refinement authorization. Missing, mismatched, ambiguous, contradictory, or aliased identity/authorization blocks before edits or persistence; never derive identity from prose, title, or path.
