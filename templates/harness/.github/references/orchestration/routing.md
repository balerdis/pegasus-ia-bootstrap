...
4. Derive launch identity deterministically. The pre-change root PRD identity is `<canonical-project-key>:prd:root`; change-scoped identities are `<change-id>:<phase>` plus `:<task-slice-id>` for Apply. No title, prose, or path-name inference is permitted.
5. Establish duplicate state before dispatch from the canonical project-scoped Pegasus Memory task/handoff state for that exact launch identity and, when change-scoped, the authorized current-change artifact state. Apply additionally uses `docs/pegasus/changes/<change-id>/apply-progress.md`. State must distinguish not-started, in-progress, and completed without stale or contradictory records. Missing/unavailable required state blocks; in-progress is recovered or awaited; completed routes only to the next authorized boundary. Never launch a duplicate.
...
8. Dispatch exactly one matching specialist with the exact canonical project key and launch identity as explicit payload fields. PRD payloads also identify root versus change-scoped persistence. If the agent tool or specialist is unavailable, launch fails, or no valid result returns, stop. Never transfer its ownership to the coordinator or another phase.
...
For PRD, require the returned project key and launch identity to exactly match the dispatch payload. A blocked awaiting-input result with concise product questions is valid only when it reports no artifact edit or persistence operation. Surface those questions unchanged, stop at the user boundary, and do not authorize Proposal. `routed` is invalid when canonical project identity, launch identity, or duplicate state is unestablished.
