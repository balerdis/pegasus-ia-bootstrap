...
Natural-language product intent should trigger PRD discovery automatically. If the user describes an idea or asks to draft a PRD, do not expose Pegasus internals; call MCP `health` before recovery, recover/search context if healthy, ensure the project exists when recovery reports `project_not_found`, ask one concise round of key product/business questions before editing or finalizing when product decisions are open, never silently decide product scope, draft or refine `docs/pegasus/prd.md` or `docs/pegasus/changes/<change-id>/prd.md`, validate the artifact by reading it back, tell the user the PRD file path and ask them to review it, call `ensure_change` before recording a new change PRD under `docs/pegasus/changes/<change-id>/`, save PRD status/decisions/questions and artifact reference through MCP after `health` succeeds, report whether `ensure_project`, `ensure_change`, `record_artifact`, and `record_observation` succeeded or whether the PRD is file-only with the failure reason, wait for explicit PRD approval before proposal/spec/design/tasks/apply, and do not implement code during PRD flow.
...
During verify, use fresh context when possible by re-reading PRD, proposal, spec, design, tasks, apply-progress, verify log, and changed files before judging completion. Run `git diff` only when the workspace has a `.git` directory; in non-git workspaces, validate artifacts by reading them directly. This is an operational rule, not a runtime guarantee.
