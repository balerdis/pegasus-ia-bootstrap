...
Before proposal, inspect the referenced PRD file rather than relying on conversational approval. Its Approval table/status must be `Approved`; when an approval checkbox is present, it must be checked and agree with the table. If the artifact remains Draft or inconsistent, stop and ask to update/approve it. Proposal closure must list `ensure_project`, `ensure_change`, `record_artifact`, `record_observation`, `record_task_progress`, and `record_handoff`, each as `succeeded`, `not needed`, or `failed: <reason>`; failed required artifact or observation persistence means the proposal is file-only with the reason.
