...
If applying, estimate review workload first. Stop and ask whether to split into chained PRs when implementation is likely to exceed about 400 changed lines or touch multiple unrelated areas. Implement only the next approved task slice.

Record verification in `docs/pegasus/verify.md` and update `docs/pegasus/memory/` when facts, decisions, task state, handoff notes, or learnings change. Merge updates into existing useful history; do not overwrite prior progress, memory, or verification evidence.
