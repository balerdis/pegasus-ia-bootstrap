...
Completed closure is atomic: finish all edits, reread the complete artifact, validate markers, language, traceability, proposal-risk coverage, deferred choices, and sources, freeze a content hash or explicit revision token, then persist `record_artifact` → `record_observation` → `record_task_progress` → `record_handoff` after ensure preconditions. No content or formatting mutation is allowed after persistence begins; a later edit invalidates prior evidence and requires full revalidation plus refreshed ordered persistence at the new revision.
