...
- AND each work unit declares implementation scope, test scope, focused test command, runtime validation, rollback boundary, `Depends on`, `Required by`, separate authored code/test/docs/config ranges including explicit zeroes, an authored total range, and a generated range or `none`
- AND estimate validation MUST parse non-negative inclusive integer minima and maxima, treat `N` as `N-N`, reject malformed or reversed ranges, require the sum of component minima to equal the work-unit minimum and the sum of component maxima to equal the work-unit maximum, then require the sums of all work-unit minima and maxima to equal the global authored forecast endpoints
- AND invalid estimates MUST be corrected before freeze followed by a full artifact reread and complete revalidation, or MUST block completion and persistence; invalid estimates MUST NOT produce `passed` validations or completed status
- AND validation constructs the work-unit dependency graph, rejects missing references, inconsistent inverse declarations, contradictions such as a unit being depended on while claiming `Required by: none`, self-dependencies, and cycles
...
- THEN the orchestrator MUST pass the active change ID and the fully expanded required canonical output path as explicit separate delegation data without prescribing forecast behavior
- AND the specialist MUST construct `Artifact path` directly from that change ID, validate exact equality with the required canonical output path before persistence/return, and MUST NOT copy a basename or path from tool output
- AND it MUST finish edits, fully reread, validate language, exact markers, current-change source identity, exactly seven forecast lines and values, exact pending evidence lines, complete work units and assigned scope, exact component and aggregate endpoint reconciliation, generated estimates, the dependency graph, and test inclusion before computing and freezing the SHA-256 final tasks revision
- AND it MUST set persistence revision equal to the frozen value before completion persistence, then ensure preconditions if needed, call `record_task_progress` carrying it, call `record_handoff` exactly once for the final revision carrying it, perform no later edit or hash recomputation, and return the envelope
...
- AND it MUST return exactly one canonical immutable v2 specialist result block bounded by `=== PEGASUS SPECIALIST RESULT BEGIN v2 ===` and `=== PEGASUS SPECIALIST RESULT END v2 ===`, containing one digest line and one canonical JSON line with the full canonical change-relative artifact path, owner/delegation fields, every validation, all seven forecast values, strategy evidence, work-unit count and assigned scope, matching final/persistence revisions, decoded post-persistence value `none`, initial recovery, ordered transitions, truthful operation states, one observable `record_handoff` invocation identity or `not observable`, risks/blockers, decision required, and next action
- AND canonical JSON MUST be UTF-8 without BOM or insignificant whitespace, use the normative ordered v2 keys with string values only, encode every `/` in every string as lowercase `\u002f`, reject malformed escapes, lone surrogates, duplicate/unknown/missing/reordered keys, and compute the lowercase SHA-256 over the exact serialized JSON line plus one final LF
- AND completed closure MUST have exactly one successful `record_handoff` invocation for the final revision when invocation count is observable; known duplicate invocations MUST be rejected, while one invocation display plus its matching result MUST be treated as one invocation
- AND the orchestrator MUST reject v1 and mixed blocks, require exactly one v2 delimiter pair, validate the digest over serialized bytes before strict decoding, require canonical reserialization byte identity, validate the decoded expected canonical path, matching artifact revisions, and closure states, then copy the entire four-line delimited block byte-for-byte into flat output without emitting a reconstructed envelope/path field
- AND omission, reordering, duplication, changed digest/payload, malformed escape, non-canonical JSON, raw slash string, decoded wrong path, missing delimiter, duplicate/mixed/legacy block, reconstructed envelope, basename/short/absolute/wrong-change/ambiguous path, unauthorized strategy, post-persistence edits, known duplicate handoff invocation, or non-truthful persistence states MUST return blocked and MUST NOT ask strategy or launch apply
- AND after the validated block it MAY state the derived guard consequence without duplicating envelope fields, then MUST reproduce exactly `La previsión requiere definir la estrategia antes de apply. ¿Elegís \`stacked-to-main\`, \`feature-branch-chain\` o una excepción \`size:exception\` aprobada por el maintainer? No se iniciará apply hasta que respondas.` without interpolation or paraphrase
- AND validators and flat transcript export contracts MUST compare that question after stripping Markdown backtick delimiters, accepting rendered/backticked and plain option tokens as equivalent while still rejecting missing, renamed, or reordered options, paraphrased decision meaning, or a missing exact no-apply sentence
