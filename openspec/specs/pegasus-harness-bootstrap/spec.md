...
- AND it MUST return the complete flat canonical envelope with the full canonical change-relative artifact path, owner/delegation fields, every validation, all seven forecast values, strategy evidence, work-unit count and assigned scope, matching final/persistence revisions, exact `Post-persistence edits: none`, initial recovery, ordered transitions, truthful operation states, one observable `record_handoff` invocation identity or `not observable`, risks/blockers, decision required, and next action
...
- AND the orchestrator MUST reproduce the entire envelope field-for-field in flat output, explicitly consume the forecast, and fail closed on omission, a basename/short, absolute, wrong-change, or ambiguous artifact path, dropped numeric fields, narrative substitution, unauthorized strategy, revision mismatch, post-persistence edits, known duplicate handoff invocation, or non-truthful persistence states
- AND before asking it MUST cite the returned authored range such as `590-720`, generated range, High risk, tests included, and work-unit count, then reproduce exactly `La previsión requiere definir la estrategia antes de apply. ¿Elegís \`stacked-to-main\`, \`feature-branch-chain\` o una excepción \`size:exception\` aprobada por el maintainer? No se iniciará apply hasta que respondas.` without interpolation or paraphrase
