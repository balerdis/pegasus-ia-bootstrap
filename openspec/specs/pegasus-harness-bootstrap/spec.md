...
- AND the orchestrator MUST reproduce the entire envelope field-for-field in flat output, explicitly consume the forecast, and fail closed on omission, a basename/ambiguous artifact path, dropped numeric fields, narrative substitution, unauthorized strategy, revision mismatch, post-persistence edits, known duplicate handoff invocation, or non-truthful persistence states
