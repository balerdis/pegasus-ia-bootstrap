...
- AND each work unit declares implementation scope, test scope, focused test command, runtime validation, rollback boundary, `Depends on`, `Required by`, separate authored code/test/docs/config ranges including explicit zeroes, a reasonably reconciled authored total range, and a generated range or `none`
...
- AND it MUST finish edits, fully reread, validate language, exact markers, current-change source identity, exactly seven forecast lines and values, exact pending evidence lines, complete work units and assigned scope, reconciled component estimates, generated estimates, the dependency graph, and test inclusion before computing and freezing the SHA-256 final tasks revision
...
- AND it MUST return exactly one canonical immutable specialist result block bounded by visible ASCII begin/end delimiter lines, with the full canonical change-relative artifact path, owner/delegation fields, every validation, all seven forecast values, strategy evidence, work-unit count and assigned scope, matching final/persistence revisions, exact `Post-persistence edits: none`, initial recovery, ordered transitions, truthful operation states, one observable `record_handoff` invocation identity or `not observable`, risks/blockers, decision required, next action, and a lowercase SHA-256 revision over the exact ordered payload bytes
...
- AND the orchestrator MUST require exactly one delimiter pair, ordered complete fields, expected canonical path, matching artifact revisions, and exact block-digest identity, then copy the entire delimited block byte-for-byte into flat output without parsing/recomposing/reformatting/shortening/linkifying/normalizing fields or emitting a second envelope/path field
- AND omission, reordering, duplication, changed digest/payload, missing delimiter, duplicate block, reconstructed envelope, basename/short/absolute/wrong-change/ambiguous path, unauthorized strategy, post-persistence edits, known duplicate handoff invocation, or non-truthful persistence states MUST return blocked and MUST NOT ask strategy or launch apply
